...
moodle_api.KEY = "30c1c7dbb82e2379fb851eb9f86720eb"
    
class User:
    def __init__(self, id, firstname, lastname, username, email, roles, key):
        self.id = id
...
def create_csv_with_users_from_course(course_id, filepath, MOODLE_API_KEY, MOODLE_URL):
    ...
    for user in course_users:
        ...
        for role in user['roles']:
            roles += role['name'] + ', '
        users.append(User(user['id'], user['firstname'], user['lastname'], "", user['email'], user['roles'][0]['name'], -1))
    ...
    for user in course_users:
        for id in id_mappings:
            if id['userid'] == user['id']:
                users.append(User(user['id'], user['firstname'], user['lastname'], user['roles'][0]['name'], -1))
                continue
            ...
            if 'fileareas' in plugin:
                for filearea in plugin['fileareas']:
                    ...
                    for files in filearea['files']:
                        ...
                        submissionURLs.append({file_name, file_url})
...
if __name__ == '__main__':
    #create_csv_with_users_from_assignment(1615, 'TANs', 'users.csv', '30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
    #insert_random_keys('users.csv')
    #upload_passwords_to_moodle(6350, 'users.csv','30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
    ...
    #print(test)
    #get_submissions_of_assignment(1615, 6427, '30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
    #course = moodle_api.call('mod_assign_get_assignments', courseids=[1615])
    #print(course)
    #moodle_api.call('core_course_get_course_module', cmid=6350)
    
    #create_csv_with_users_from_course(1615, 'users.csv', '30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
    ...
    params2 = {}
    params2['workshopid'] = 6350
    params2['userid'] = 18141
    #subs = moodle_api.call('mod_workshop_get_submission', **params2)
    #print(subs)
    #print(submission)
    
    #create_csv_with_users_from_course(1615, 'users.csv', '30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
    
    #get_submissions_of_assignment(1615, 6350, '30c1c7dbb82e2379fb851eb9f86720eb', 'https://moodle.informatik.tu-darmstadt.de')
