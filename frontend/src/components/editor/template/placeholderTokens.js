 ...
 * Duplicated from backend/utils/placeholderTokens.js because the browser cannot import backend Node modules.
 *
 * @author Mohammad Elwan
 */

/** Matches ~placeholderKey~ and ~placeholderKey[N]~. */
export const PLACEHOLDER_TOKEN_REGEX = /~([A-Za-z0-9_]+)(?:\[(\d+)\])?~/g;
 ...
 */
export function parsePlaceholderMatch(match) {
  if (!match || !match[1]) {
 ...
 */
export function formatPlaceholderToken(baseKey, index) {
 ...
 */
export function tokenInnerText(baseKey, index) {
  return `${baseKey}[${index}]`;
}
 ...
 */
export function getUsedIndexes(text, baseKey) {
  if (!text || !baseKey) {
    return [];
  }
  const indexes = [];
 ...
 */
export function getNextPlaceholderIndex(text, baseKey) {
 ...
 */
export function countPlaceholdersByKey(text, options = {}) {
 ...
 */
export function getDuplicatePlaceholderIndexes(text, baseKeyFilter) {
  const seen = new Map();
  const duplicates = [];
  if (!text) {
    return duplicates;
 ...
 */
export function formatDuplicatePlaceholderToken(entry) {
 ...
 */
export function applyPlaceholderReplacements(text, resolveValue) {
