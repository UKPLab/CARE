...
function validateConfig(cfg) {
    const errors = [];
    if (!['ramp', 'soak', 'regression'].includes(cfg.mode)) errors.push(`--mode must be "ramp", "soak", or "regression" (got "${cfg.mode}")`);
...
async function run(cfg) {
    ...
    const emit = (event, payload) => emitWithAck(socket, event, payload);
