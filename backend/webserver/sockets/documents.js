...
exports = module.exports = function(io) {
    ...
    io.on("connection", (socket) => {
        ...
        const update_documents = uid => {
            pdb.query('SELECT * FROM public."document" WHERE "creator" = $1 and "deleted" = False',
                [ uid ])
            .then((rows) => {
        ...
        socket.on("docs_get", (data) => {
            update_documents(socket.request.session.passport.user.uid);
        });
        ...
        socket.on("doc_delete", (data) => {
            ...
            console.log(data);
                deleteDoc(data.docId)
                    .then(() => update_documents(socket.request.session.passport.user.uid))
        ...
        socket.on("doc_upload", (data) => {
            ...
            addDoc(name, socket.request.session.passport.user.uid)
                .then((pdf_id) => {
