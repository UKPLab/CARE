...
module.exports = (sequelize, DataTypes) => {
    class Document extends MetaModel {
         ...
         * @param {Object} includes - Optional includes for extra associations (e.g., { studySessionId: 45 }) duplicate includes data that include studySessionId
         * @param {Object} options - Database options including transaction
         * @returns {Promise<Object>} The duplicated document
         */
        static async duplicateDocument(documentId, overrides = {}, includes = {}, options = {}) {
            ...
            await sequelize.models.document_data.duplicateDocumentData(originalDoc.id, duplicatedDoc.id, includes, transaction);

            if (originalDoc.type === Document.docTypes.DOC_TYPE_PDF) {
                await sequelize.models.annotation.duplicateAnnotations(originalDoc.id, duplicatedDoc.id, includes, transaction);
            }
            else if(originalDoc.type === Document.docTypes.DOC_TYPE_HTML || originalDoc.type === Document.docTypes.DOC_TYPE_MODAL){
                await sequelize.models.document_edit.duplicateEditsByDocument(originalDoc.id, duplicatedDoc.id, includes, transaction);
            }
