...
that must stay safe, use the vue-i18n :code:`<i18n-t>` 
...
* **Migrations** — when ``up`` stores i18n keys in the database (e.g. ``placeholderLabel``), ``down`` calls
  ``translateMaybeKey`` to write back the English source string. If the key is missing from ``en`` JSON, the
  key string is returned unchanged (safe for rollback).
