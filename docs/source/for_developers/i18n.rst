...
.. code-block:: javascript
   ...
   // What the frontend receives in (response) => { ... }:
   {
     ...
     params: { id: userId },
     message: "User with id 123 was not found.",  // English fallback (resolveLogText)
...
Server logs and the dashboard log table stay **English**. If you log an i18n key as-is, the log shows the
raw key instead of readable text. Resolve it first with ``resolveLogText(message, params?)`` from
...
.. code-block:: javascript
   ...
   // Before — raw key in the log:
   this.logger.error(key);

   // After — readable English (params filled in when the key has placeholders):
   this.logger.error(resolveLogText(key));
   this.logger.error(resolveLogText("errors.auth.passwordResetRateLimited", { minutes: 5 }));

   // Plain English or non-key strings pass through unchanged:
   this.logger.error(resolveLogText("Import finished with warnings"));
