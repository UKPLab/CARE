...
help:
	...
	@echo "make audit            				npm audit for frontend, backend, utils/modules/editor-delta-conversion, utils/modules/placeholder-tokens"
...
ifeq ($(OS),Windows_NT)
audit:
	@powershell -NoProfile -Command "$$e=0; foreach ($$p in @('frontend','backend','utils/modules/editor-delta-conversion','utils/modules/placeholder-tokens')) { Write-Host ''; Write-Host ('=== npm audit: ' + $$p + ' ==='); npm.cmd audit --prefix (Join-Path '$(CURDIR)' $$p); if ($$LASTEXITCODE -ne 0) { $$e=1 } }; exit $$e"
else
audit:
	@st=0; for d in frontend backend utils/modules/editor-delta-conversion utils/modules/placeholder-tokens; do \
